...
var RichContentView = (_temp2 = _class = function (_React$Component) {
            ...
            var _loop = function _loop(i) {
                ...
                var onImgClick = function onImgClick() {
                    if (_this.props.onImageClick) {
                        _this.props.onImageClick(i, img.url, newImgEl);
                    } else {
                        (0, _ImageViewer2.default)(i, _this.imgs, newImgEl);
                    }
                };
    ...
    _createClass(RichContentView, [{
        ...
        value: function render() {
            ...
            var data = this.state.content.replace(/font-size:\s*\d+(\.\d+)?px;/gi, '').replace(/font-size:\s*\d+(\.\d+)?pt;/gi, '').replace(/ alt=""/gi, '');
            var result = '';
            this.imgs = [];
            var currentId = 0;
            while (data.length > 0) {
